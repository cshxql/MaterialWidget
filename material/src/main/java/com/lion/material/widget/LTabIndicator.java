...
public class LTabIndicator extends HorizontalScrollView {
	...
	private int indicatorColor = 0xdeffffff;
	private int underlineColor = 0x8AFFFFFF;// 0x42ffffff;
	private int dividerColor = 0x1A000000;
	private int textSelectedColor = 0xDEFFFFFF;
	private int textUnselectColor = 0x8AFFFFFF;
	private int mAnimColor = 0x8affffff;
	...
	private float indicatorHeight = 3f;
	...
	public LTabIndicator(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public LTabIndicator(Context context, AttributeSet attrs, int defStyle) {
		...
		mBaseLinePaint.setStyle(Style.FILL);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (indicatorOnTop) {
			...
			canvas.drawRect(0, height - underlineHeight,
					mTabsContainer.getWidth(), height, mBaseLinePaint);
		}
		...
		if (indicatorOnTop) {
			canvas.drawRect(lineLeft, 0, lineRight, indicatorHeight,
					mCurrentPaint);
		} else {
			canvas.drawRect(lineLeft, height - indicatorHeight, lineRight,
					height, mCurrentPaint);
		}
	...
	private class TabView extends LFrameLayout {
		...
		public TabView(Context context, AttributeSet attrs) {
			super(context, attrs);
			init();
			this.setDelayClick(false);
			this.setColor(mAnimColor);
		}

		private void init() {
			mTabText = new TextView(getContext());
//			mTabText.setTextAppearance(getContext(),
